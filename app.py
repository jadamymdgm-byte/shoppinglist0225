...
/* カラムが押し潰されてもはみ出ないように設定 */
div[data-testid="stHorizontalBlock"] > div[data-testid="column"] {
    min-width: 0 !important;
    padding: 0 3px !important;
}

/* プルダウンの「在庫」「必要」というラベルを隠す */
.stSelectbox label {
...
.row-buy { background-color: #fefcbf; } /* 買うものがある時は黄色 */
.row-ok  { background-color: #ffffff; } /* 買うものがない時は白 */
</style>
...
# --- 4. 買い物リスト表示 ---
# ヘッダー作成 (横幅の比率 → 品名:3.5, 在庫:1.5, 必要:1.5, 操作:2.0)
h1, h2, h3, h4 = st.columns([3.5, 1.5, 1.5, 2.0])
with h1: st.markdown('<div class="header-col left">品名</div>', unsafe_allow_html=True)
with h2: st.markdown('<div class="header-col">在庫数</div>', unsafe_allow_html=True)
with h3: st.markdown('<div class="header-col">必要数</div>', unsafe_allow_html=True)
...
if not items:
    ...
    for item in items:
        ...
        # 背景色の設定
        row_class = "row-buy" if is_needed else "row-ok"
        
        # 1行のコンテナ開始
        st.markdown(f'<div class="row-container {row_class}">', unsafe_allow_html=True)
        
        # 列の配置
        c1, c2, c3, c4 = st.columns([3.5, 1.5, 1.5, 2.0], vertical_alignment="center")
        ...
        with c4:
            if is_needed:
                # 購入ボタン：在庫＋必要数を足して、必要数を0にする (最大値は15)
